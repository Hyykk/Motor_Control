...
#define Kcp 0.827        // 전류제어기 P gain	(La*wcc) => wcc=2pi*fcc
#define Kci 2.2117e+03    // 전류제어기 I gain	(Ra/La)*Kp
#define Kac 1.209         // 전류제어기 anti wind up (1/Kcp)

#define Kvp 1.5				//  속도제어기 P gain	(J*wcv)/Kt => Trial and error로 구함
#define Kvi 40.0			//  속도제어기 I gain (B/J)*Kp => 공식을 이용해 구한 게인 값을 넣으면 과한 진동
...
#define Kpd 0.1           //  위치제어기 D gain	Kpd=wcp/wcs=1/10
...
ISR(TIMER0_OVF_vect){
	...
	g_Pcur = (double)(g_Cnt / (4096. * 81.)) * 2 * M_PI;   // Unit : [rad]
